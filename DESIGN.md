...
> DSH 主题插件（`dsh-yoimiya-theme`）——以《原神》宵宫 / 长野原烟花店为母题，
> 面向**长时间盯屏**的护眼配色与背景。
> Web 端与 DSH Desktop 共用同一套界面，因此本方案对两者同时成立。
...
上面几层都是静态的；真正每帧在变的只有这一层：一张全屏 Canvas 2D，纯装饰，
任何一步失败都只是「没有粒子」，绝不影响主题。

**模拟**（`createParticles()`）：

| 阶段 | 做法 |
| --- | --- |
| 引线 | 随机取一个目标高度（视口高的 66%–92%，即炸在屏幕上方 8%–34%），再由「升多久」反推上升速度 |
| 爆炸 | 均匀分布一圈火星（16–38 个 × 密度档），每个带随机初速 |
| 火星 | 受重力下坠、受空气阻力减速，按寿命淡出 |
| 拖尾 | 每帧先用 `destination-out` 铺一层半透明黑淡出上一帧，再用 `lighter` 叠加本帧——画布其余部分保持透明，所以能当背景层用 |

**为什么速度档只作用于升起**：早先的写法把速度档乘在寿命 `dur` 上、而上升速度
`vy` 是独立随机的，结果是「调快档只让它提前炸开（炸得更低），升起速度纹丝不动」。
现在先随机目标高度、再由 `dur` 反推 `vy`，速度档才真的作用在升起上。
爆炸档则同时放大扩散半径与火星寿命——两者一起变才是"炸得大"，只放大其中一个会
显得不自然（大而短、或小而久）。

**调色板**与主题 token 同源（引线橙 / 暖金 / 朱红 / 淡金），避免出现"外部插件感"
的荧光色。

**三个档位**（升起速度 / 密度 / 爆炸）各三档，存 `localStorage`
（`dsh-yoimiya-particles-v1`）。默认尊重系统的「减少动态效果」：系统要求减少动效
且用户没有表过态时，默认关闭。

**什么时候停**：切到后台（`visibilitychange`）、用户关掉开关、插件卸载。面板底部
有一行「开销」读数，见 §12.4。
