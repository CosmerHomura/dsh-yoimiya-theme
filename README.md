...
**代码与程序化美术**（`bundle/`、`tools/`、`bg-night.svg`、`bg-day.svg`、
`mark.svg`、`mark-day.svg`）：MIT。这些是我原创的手写 SVG 与样式，可自由使用。

**`assets/yoimiya-wide.jpg` 与 `assets/yoimiya.jpg`**：由你提供，是《原神》的角色
美术，**不在 MIT 授权范围内**。本包只把它们当作本地个人主题的壁纸使用。
如果要公开分发这个主题，请换成你有权分发的素材——删掉这两张图，以及
`bundle/host.js` 里对应的两条路由，并移除 `bundle/client.js` 中的 `body::before`
规则块；主题会自动退回纯程序化背景，其余功能不受影响。

这两张图已通过 `.gitignore` 排除，**不随仓库分发**。所以从仓库安装的人拿到的是
无壁纸版本：`body::before` 指向的资源 404，该层不绘制，底下的程序化霞夜天空
照常显示——配色、遮罩、语法高亮、金鱼标识全部正常，只是少了人物立绘。
想补上，把自己的图放到 `assets/yoimiya-wide.jpg` 即可（16:9，人物放左侧三分之一
最合适，理由见上文「换成你自己的壁纸」）。
